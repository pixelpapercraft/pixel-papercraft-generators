...
test("North: Full/Left/Middle/Right render distinct outer-edge spans", async ({
  ...
  // Full: row 80 (the rectangle's own top row, inside its blue fill).
...
test("South: Full/Left/Middle/Right render distinct outer-edge spans", async ({
  page,
}) => {
  await selectOrientation(page, "South");

  // Full/Left/Middle: row 109 (the rectangle's own bottom row, inside).
...
test("West: Full/Left/Middle/Right render distinct outer-edge spans", async ({
  page,
}) => {
  await selectOrientation(page, "West");

  // Full/Right: column cellX (inside the rectangle's fill).
