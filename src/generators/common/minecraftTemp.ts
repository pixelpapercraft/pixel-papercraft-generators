...
export function translateRectangle(
  rectangle: Rectangle,
  [xTranslate, yTranslate]: [number, number]
): Rectangle {
  const [x, y, w, h] = rectangle;
  return [x + xTranslate, y + yTranslate, w, h];
}
...
export function makeCuboid([w, h, d]: Scale): Cuboid {
  return {
    top: [d, 0, w, d],
    bottom: [d + w, 0, w, d],
    front: [d, d, w, h],
...
export function translateCuboid(
  source: Cuboid,
...
function flipFaceVertical(face: Face): Face {
  switch (face.flip) {
    ...
    case "Horizontal":
      return { ...face, flip: "None", rotate: face.rotate + 180.0 };
  }
}
...
export function rotateFace(face: Face, r: number): Face {
  ...
  return {
    rectangle:
      r % 180 === 90 ? [x + (w - h) / 2, y - (w - h) / 2, h, w] : [x, y, w, h],
    flip: face.flip,
...
//     let draw = (textureId: string, source: Builder.rectangle, dest: Cuboid) => {
